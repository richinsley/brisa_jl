...
private:
     ...
     */
    // int requestId;
    // jl - we'll cast the pointer to the actual QNetworkReplay and use that as the request ID
     ...
     */
    HHttpRequestHeader *getSubscriptionRequest(const int timeout);

    /*!
     *    Creates and returns an unsubscription Http header.
     *    \param unsubscription Http header
     */
    HHttpRequestHeader *getUnsubscriptionRequest() const;
     ...
     */
    HHttpRequestHeader *getRenewRequest(const int timeout) const;
