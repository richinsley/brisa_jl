...
{
    // jl - multicast supported since 4.8
    // jl - TODO - implement IPv6
    bind(QHostAddress::AnyIPv4, port, QUdpSocket::ShareAddress);
    joinMulticastGroup(QHostAddress(address));
    ...
    qWarning() << "windows procedure is running...";
       ...
       mreq.imr_multiaddr.s_addr = inet_addr(address.toLatin1());
       ...
       if(!this->setSocketDescriptor(fd,QUdpSocket::BoundState,QUdpSocket::ReadOnly))
       {
           qWarning("BrisaUdpListener::start failed to set spcket descriptor");
       }
#endif
