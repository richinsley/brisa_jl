...
BrisaControlPoint::BrisaControlPoint(QObject *parent, QString st, int mx) :
    ...
    //this->http = new QHttp();
    // jl
    this->httpm = new QNetworkAccessManager();
    ...
    //jl
    connect(httpm, SIGNAL(finished(QNetworkReply *)), this, SLOT(httpFinished(QNetworkReply *)));
...
// jl
void BrisaControlPoint::httpFinished(QNetworkReply *reply) {
    qDebug() << "httpFinished";
    qulonglong i = (qulonglong)reply;
    ...
    if (!subscription) {
        ...
        qWarning() << "Brisa Control Point: Subscription error "  << reply->errorString() << " : " << i;
        return;
    }

    QList<QByteArray> hlst = reply->rawHeaderList();
    QString sid = "";
    if(hlst.contains("SID"))
    {
        sid = reply->rawHeader("SID");
    }
    else if(hlst.contains("sid"))
    {
        sid = reply->rawHeader("sid");
    }
    ...
    if (sid.isEmpty()) {
        ...
        qWarning() << "Brisa Control Point: SID header not present on event subscription response. " << i;
        foreach(QString key, hlst) {
            qDebug() << key << reply->rawHeader("sid");
        }

        qDebug() << "Brisa Control Point: Finished printing headers.. printing data" << reply->readAll();
