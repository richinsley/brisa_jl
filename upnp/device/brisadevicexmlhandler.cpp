    ...
    foreach(BrisaService *s, serviceList)
        {
            ...
            // jl - need to properly encode serviceID
            QString vendor = s->getAttribute(BrisaService::Vendor);
            if(vendor.isEmpty())
            {
                vendor = "upnp-org";
            }
            this->writer->writeTextElement("serviceId", "urn:" + vendor + ":serviceId:" + s->getAttribute(
