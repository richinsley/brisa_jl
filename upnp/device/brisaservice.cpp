    ...
    for (QList<BrisaAction *>::iterator i = this->actionList.begin(); i != actionList.end(); ++i) {
        ...
        if (action->getName() == method) {
            ...
            if (this->preActionMethod.methodIndex() >= 0) {
                ...
                {
                    qDebug() << "Error invoking preAction method [" << method << "] Continuing...";
                }
            } else {
                qDebug() << "Warning: preAction method [" << method << "] not found, continuing...";
            }
            ...
            if (prePostActionReturn == 0) {
                ...
                if (!action->call(&param, outArguments)) {
                    ...
                    if (this->handleActionFailureMethod.methodIndex() >= 0) {
                        ...
                        if (!this->handleActionFailureMethod.invoke(this,
                                                                    ...
                                                                    Q_ARG(QString, errorDescription)))
                        {
                            qDebug() << "Error invoking handleActionFailure method [" << method << "] . Continuing...";
                ...
                if (this->postActionMethod.methodIndex() >= 0) {
                    if (!this->postActionMethod.invoke(this,
                                                       ...
                                                       Q_ARG(QString, errorDescription)))
                    {
                        qDebug() << "Error invoking postAction method [" << method << "].";
                    }
                    ...
                    qDebug() << "Warning: postAction method [" << method << "] not found, continuing...";
                }
                ...
                qDebug() << "Warning: preAction method [" << method << "] returned non-zero value, it returned " << prePostActionReturn << " with description " << errorDescription;
                respondError(session, prePostActionReturn, errorDescription);
                return;
            }
        }
...
void BrisaService::parseDescriptionFile() {
    ...
    {
        qDebug() << "BrisaService failed to load service description file: " << this->scpdFilePath << " : " << file.errorString();
        return;
    }
