...
void BrisaSSDPClient::datagramReceived() {
    while (this->udpListener->hasPendingDatagrams()) {
        ...
        int psize = udpListener->pendingDatagramSize();
        datagram->resize(psize);
        int s = datagram->size();
        char * ddata = datagram->data();
        udpListener->readDatagram(ddata, s);

        QString temp(datagram->data());

        HHttpRequestHeader *parser = new HHttpRequestHeader(temp);
...
void BrisaSSDPClient::notifyReceived(HHttpRequestHeader *datagram) {
